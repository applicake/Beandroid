...
public class AccountSettingsActivity extends BeanstalkActivity implements OnClickListener {
			...
			progressDialog.setOnCancelListener(new OnCancelListener() {
				...
				public void onCancel(DialogInterface dialog) {
					...
					GUI.displayMonit(mContext,
							"Downloading account information task was cancelled");
				}
			});
		}
		...
		protected void onPostExecute(Integer result) {
			...
			if (failed) {
				...
				if ((result == 200) && (account != null)) {
					...
					timezoneSpinner.setSelection(spinnerValuesList.indexOf(account
							.getTimeZone()));
				} else if (result == 0) {
	...
	public class SendUpdateAccountPropertiesTask extends AsyncTask<Void, Void, Integer> {
		...
		protected Integer doInBackground(Void... params) {
			...
			try {
				String accountModificationXml = xmlCreator
						.createAccountPropertiesChangeXML(accountName.getText()
								.toString().trim(), spinnerValuesList.get(timezoneSpinner
