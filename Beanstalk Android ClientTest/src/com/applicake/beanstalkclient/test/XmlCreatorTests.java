...
public class XmlCreatorTests extends AndroidTestCase {
	...
	public void testCreateCommentXml() throws IllegalArgumentException,
		...
		assertEquals("<comment>" + "<revision>1</revision>"
				+ "<body>comment 1 body</body>" + "<file-path></file-path>"
	...
	public void testCreateGitRepositoryXML() throws IllegalArgumentException,
		...
		assertEquals("<?xml version='1.0' encoding='UTF-8' ?>" + "<repository>"
				...
				+ "<color_label>label-pink</color_label>" + "</repository>",
				createGitRepoXML);
	...
	IllegalStateException, IOException {
		...
		assertEquals("<?xml version='1.0' encoding='UTF-8' ?>" + "<repository>"
				...
				+ "<color_label>label-blue</color_label>" + "</repository>",
				createGitRepoXML);
		
	}
	

	public void testModifyReposiotoryXML() throws IllegalArgumentException, IllegalStateException, IOException {
	xmlCreator = new XmlCreator();
		
		String title = "test repository new title";
		String colorLabel = ColorLabels.GREY.getLabel();
		
		String createGitRepoXML = xmlCreator.createRepositoryModifyXML(title,
				colorLabel);
		
		assertEquals("<?xml version='1.0' encoding='UTF-8' ?>" + "<repository>"
				+ "<title>test repository new title</title>"
				+ "<color_label>label-grey</color_label>" + "</repository>",
				createGitRepoXML);
